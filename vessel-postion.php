...
try {
    ...
    $port = (new Query('port'))
        ->setSelectionSet(['unlocode', 'name']);
    
    $matchedPort = (new Query('matchedPort'))
        ->setSelectionSet(['matchScore', $port]);



    // Build dimensions query
    $currentVoyage = (new Query('currentVoyage'))
        ->setSelectionSet(['destination', 'draught', 'eta', 'timestamp', 'updateTimestamp', $matchedPort]);    
    
    
    // Build validated query
    $lastPositionUpdate = (new Query('lastPositionUpdate'))
        ->setSelectionSet([
            'accuracy',
            'collectionType',
            'course',
